    ...
    return (
        <Section className="px-4">
            <div className="text-center mb-16 md:mb-24">
                <span className="py-1.5 px-4 rounded-full bg-brand-yellow text-black text-[10px] md:text-sm font-black uppercase tracking-wider mb-8 inline-block">
                    🎁 INSANE VALUE STACK!
                </span>
                <h2 className="text-3xl md:text-7xl font-outfit font-black mb-6 leading-[1.1]">
                    Get My ₹4,999 Growth <br className="md:hidden" /> Bundle for <br className="hidden md:block" />
                    the Price of a <br className="md:hidden" /> <span className="text-brand-yellow glow-text">Cutting Chai!</span>
                </h2>
            ...
            <motion.div
                ...
                viewport={{ once: true }}
                className="bg-brand-yellow rounded-3xl md:rounded-[4rem] p-8 md:p-20 text-center text-black shadow-[0_30px_60px_rgba(250,204,21,0.3)] relative overflow-hidden"
            >
                {/* Decorative Elements */}
                ...
                <div className="relative z-10">
                    <p className="uppercase font-black tracking-[0.2em] text-[10px] md:text-sm mb-4 opacity-70">Total Value:</p>
                    <p className="text-3xl md:text-6xl font-black line-through opacity-30 mb-8 tracking-tighter">₹4,996</p>
                    <p className="uppercase font-black tracking-[0.2em] text-[10px] md:text-sm mb-4">Today's Price:</p>
                    <p className="text-8xl md:text-[12rem] font-black mb-12 drop-shadow-2xl tracking-tighter leading-none">₹49</p>
                    <p className="text-sm md:text-xl font-bold max-w-lg mx-auto mb-12 opacity-80 leading-relaxed px-4">
                        Why ₹49? To keep the time-wasters away and only talk to serious action-takers like you.
                    </p>
                    <button className="cta-button-trigger w-full md:w-auto bg-black text-white px-8 md:px-16 py-5 md:py-8 rounded-2xl text-xl md:text-3xl font-black uppercase tracking-tight hover:scale-[1.03] active:scale-95 transition-all flex items-center justify-center gap-4 mx-auto shadow-2xl group">
                        Claim This Deal Now <span className="group-hover:translate-x-2 transition-transform">→</span>
                    </button>
                    <p className="mt-8 text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] opacity-40">Limited Time Offer • Secure Checkout</p>
                </div>
            </motion.div>
        </Section>
    );
